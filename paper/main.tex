...
\begin{figure}[!t]
\centering
\includegraphics[width=0.70\textwidth]{figures/qualitative.pdf}
...
A false positive in $m_t$ does not by itself produce a synthetic lesion, for two reasons. The generator trains on \emph{predicted} masks, so some $m_t{=}1$ regions it saw carried no enhancement in $y$; it learns to treat $m_t$ as a prior to reconcile with the pre-contrast appearance rather than as an instruction. And in residual mode identity is free --- $\Delta{=}0$ reproduces $x$ --- so non-zero $\Delta$ must be earned against masked $L_1$ and perceptual terms that penalize enhancement wherever $y \approx x$. Together these bias the network towards modulating structure already visible in $x$ rather than inventing it. This is a bias, not a guarantee: adversarial training can still synthesize plausible texture, and clinical use would need a dedicated false-positive study.

\paragraph{Limitations.}
MSE is dominated by outliers, which a robust regression term would temper, and our breast-masked training loss is mismatched with the full-image evaluation protocol --- a low-weight full-image consistency term, or an evaluation mask weighted by anatomical relevance, would close that gap. On the MSEC finding, we tested a single weight and a single formulation, so a smaller $\lambda_{MSEC}$ or a tumor-restricted variant may still help; nor does our result speak to the multi-parametric setting in which the loss was validated~\cite{yang2026miccai}. A weight sweep, model ensembling, and multi-resolution processing are left to future work.
