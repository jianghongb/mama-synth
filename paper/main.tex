...
The SUB-DDPM numbers in Table~\ref{tab:merged_results} are \emph{not} from the challenge leaderboard: they are our own adaptation of the subtraction-based conditional diffusion model of Ibarra et al.~\cite{ibarra2025diffusion}, run on the same 299 test slices. We reuse the authors' released implementation unchanged --- the 8.4M-parameter \texttt{ConditionalUNet}, $x_0$-prediction under a cosine$^2$ schedule, the target $\mathrm{clip}((y-x)/0.5,-1,1)$, and their composite MAE/VGG/TV/MSE objective --- and sample with 50 denoising steps. Our one substantive departure is the input pipeline, and it is the likely explanation for the gap: the original normalizes each slice by min--max, preserving its full dynamic range, whereas we kept the globally z-scored representation used throughout this work and mapped it to $[-1,1]$ by clipping at $\pm 3$. With test intensities reaching $\approx 18$, that clipping discards much of the signal the model is asked to synthesize. Nothing obliged us to keep that representation --- our own generator re-normalizes per image --- but we did not re-tune the baseline's preprocessing to its original assumptions. This row should therefore be read as a statement about that adaptation, namely that per-slice min--max normalization is load-bearing for this model family, rather than as a verdict on conditional diffusion. The baseline also saw 1,194 training slices against our 9,504, so the comparison is not compute-matched either.
