...
Neither conditioning mask can leak the synthesis target, but for different reasons. The breast segmentor $m_b$ is a 2D nnU-Net distilled from public pseudo-labels --- the BreastDivider dataset (\texttt{Bubenpo/BreastDividerDataset}) and the 3D teacher that produced them (\texttt{ykirchhoff/Bre\-astDividerModel})~\cite{rokuss2025breastdivider} --- on mid-axial slices. That corpus is anonymised and may well include DUKE or ISPY cases from our test split, but $m_b$ is a breast-versus-background mask read off pre-contrast anatomy: it carries no contrast or lesion information and cannot convey the post-contrast target. Separation matters for $m_t$, and there it is strict: its nnU-Net dataset was built exclusively from the 9,504 train-split slices, so none of the 60 test patients contribute a slice, and the separation asserted for the evaluation segmenter (Sec.~\ref{subsec:experiment_setting}) holds here too. On its own validation fold (1,901 slices) it reaches a foreground Dice of 0.767.
