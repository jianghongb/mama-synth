...
### 方案 A: mask_and_preprocess.py (一体化, Dataset932 4ch)

```
┌─────────────────────────────────────────────────────────────────┐
│              mask_and_preprocess.py (GPU, ~19h)                   │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
...
│    ├── ground_truth/  (2D subtraction, masked, z-score)         │
│    ├── mask/          (2D tumour mask)                           │
│    └── breast_mask/   (2D breast mask)                          │
│                                                                 │
│  优点: 更精确 (4ch 专业乳腺分割)                                  │
│  缺点: 慢 (~19h GPU), 需要 post-contrast 数据                    │
└─────────────────────────────────────────────────────────────────┘
```
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Step 1: preprocess.py (CPU, ~30min)                            │
│    images/ + segmentations/ → 2D MHA                            │
│    (3D NIfTI → 选 peak phase → 选 tumor 最大 slice → z-score)   │
│    不做 breast masking                                           │
│                                                                 │
│  Step 2: generate_breast_masks.py --model_type 910 (GPU, ~75min)│
│    mha/input/ ──→ Dataset910 (ResEncUNet 2D, 单通道 T1)         │
...
│                                                                 │
│  优点: 快 (2h vs 19h), 推理时也能用同一模型                       │
│  缺点: 精度略低 (10类通用模型 vs 3类专业模型)                      │
└─────────────────────────────────────────────────────────────────┘
```
