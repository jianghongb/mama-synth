...
class Pix2PixHDModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.use_tumor_loss = getattr(opt, 'tumor_weight', 0) > 0
            self.tumor_weight = getattr(opt, 'tumor_weight', 0)
            self.lambda_vgg = getattr(opt, 'lambda_vgg', 10.0)
            self.lambda_ssim = getattr(opt, 'lambda_ssim', 0)
    ...
    def forward(self, label, inst, image, feat, infer=False, mask=None):
        ...
        if self.use_ssim_loss:
            ...
            # z-score data: typical range ~[-2, 6], use dynamic range
            data_range = real_image.max() - real_image.min()
            if data_range > 0:
        ...
        # MSSC: Multi-Scale Subtraction Consistency loss
        loss_G_MSSC = 0
        if self.use_mssc_loss:
