...
#
# v31: UC-GAN (v28) + Swin Transformer bottleneck (v29) combined.
# Generator: shared encoder → Swin blocks (long-range) → dual head (mu + log_var).
# Discriminator judges high-confidence regions only.
# Data: data_multislice_v2/train + LAB.
...
echo "=== Starting v31 training (Swin + UC-GAN, batch=16, nThreads=16) ==="
...
echo "Done! Weights: $PROJ/checkpoints/mamasynth_v31/latest_net_G.pth"
