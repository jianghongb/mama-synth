...
#
# v9: v5 (MSEC=50, data_split 1074 cases) + breast mask (loss only in breast region)
# Purpose: combine v5's clean data with v7's breast masking advantage
...
# Step 1: Generate breast masks for data_split/train (one-time)
if [ ! -d "$PROJ/data_split/train/mha/breast_mask" ]; then
    echo "Generating breast masks for data_split/train..."
    python $PROJ/mama-synth/src/submission/submission-synthesis/models/generate_breast_masks.py \
        ...
        --output_dir $PROJ/data_split/train/mha/breast_mask \
...
python train.py \
  ...
  --num_D 2 \
  --n_layers_D 3 \
  --breast_mask_dir $PROJ/data_split/train/mha/breast_mask \
  --save_epoch_freq 5 \
  --print_freq 100 \
  --gpu_ids 0
