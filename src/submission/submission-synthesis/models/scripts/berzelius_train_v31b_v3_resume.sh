...
#
# v31b_v3 RESUME: continue training from last saved checkpoint.
# Original job 17026520 failed after 17h47m.
...
echo "=== Resuming v31b_v3 training ==="
echo "Checkpoint dir: $PROJ/checkpoints/mamasynth_v31b_v3/"
ls -la $PROJ/checkpoints/mamasynth_v31b_v3/latest_net_G.pth 2>/dev/null && echo "latest_net_G.pth exists" || echo "WARNING: no checkpoint found!"

python train.py \
  --name mamasynth_v31b_v3 \
  --model pix2pixHD \
...
echo "Done! Weights: $PROJ/checkpoints/mamasynth_v31b_v3/latest_net_G.pth"
