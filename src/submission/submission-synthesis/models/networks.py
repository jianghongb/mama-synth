...
class VGGLoss(nn.Module):
    ...
    def forward(self, x, y):
        # Handle single-channel input: replicate to 3 channels for VGG
        if x.shape[1] == 1:
            x = x.repeat(1, 3, 1, 1)
            y = y.repeat(1, 3, 1, 1)
...
class MSScLoss(nn.Module):
    ...
    def forward(self, fake, real, pre):
        """
        Args:
            fake: generated post-contrast [B,1,H,W]
            real: ground truth post-contrast [B,1,H,W]
            pre:  pre-contrast input [B,1,H,W]
        """
        sub_fake = fake - pre
        sub_real = real - pre
        loss = 0
